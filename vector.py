...
class Vector(object):
    ...
    def subtract(self,newVector):
        ...
        #to do should check to see if lengths are the same first
        #print(self.coordinates,newVector.coordinates)
        #sys.exit(0)
        newSum = [(Decimal(x)-Decimal(y)) for x,y in zip((self.coordinates),(newVector.coordinates))]
        #Convert list to a Vector object
        newVector = Vector(newSum)
        #print (newVector)
        return newVector
	...
	#to do should check to see if lengths are the same first
	    listProduct = [(Decimal(x)*Decimal(y)) for x,y in zip(self.coordinates,newVector.coordinates)]
